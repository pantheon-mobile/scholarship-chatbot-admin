...
export async function fetchFaqs(filters: FaqFilters): Promise<FaqListResponse> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs?${queryString(filters)}`, { cache: "no-store" });
...
export async function createFaq(values: FaqCreate): Promise<FaqDetail> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs`, {
...
export async function fetchFaq(id: number): Promise<FaqDetail> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs/${id}`, { cache: "no-store" });
...
export async function updateFaq(id: number, values: FaqUpdate): Promise<FaqDetail> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs/${id}`, {
...
export async function deleteFaq(id: number, version: number): Promise<void> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs/${id}?version=${version}`, { method: "DELETE" });
...
export async function bulkDeleteFaqs(items: Array<{ id: number; version: number }>): Promise<number> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs/bulk-delete`, {
...
export async function exportFaqs(filters: FaqFilters): Promise<Blob> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs/export?${queryString(filters, false)}`);
...
export async function downloadFaqImportTemplate(): Promise<Blob> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs/import-template`);
...
export async function importFaqs(file: File): Promise<FaqImportResponse> {
  ...
  const response = await authenticatedFetch(`${apiBase}/api/v1/faqs/import`, { method: "POST", body: formData });
