...
export default function DataSourceWebsiteEditPage() {
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!row || !dirty || !values.url.trim() || busy) return;
    ...
    try {
      ...
      if (err instanceof DataSourcesApiError && err.code === "VERSION_CONFLICT") {
        ...
        setError(err instanceof Error ? err.message : "Webサイトの更新に失敗しました。");
      }
    } finally {
      setBusy(false);
    }
    ...
    <AdminLayout activeMenu="data-sources" contentWidth="wide" contentAlign="start" onNavigate={requestNavigate}>
      <form className={styles.page} onSubmit={submit}>
        <div className={styles.topRow}>
          <Breadcrumb items={[{ label: "データソース一覧", onClick: () => requestNavigate("/data-sources") }, { label: "データソース情報編集（Webサイト）" }]} />
      ...
      <Modal open={leaveOpen} title="確認" confirmLabel="一覧に戻る" busy={busy} closeOnBackdrop={!busy} closeOnEscape={!busy} onClose={() => { if (!busy) setLeaveOpen(false); }} onConfirm={() => { if (!busy) { setLeaveOpen(false); router.push("/data-sources"); } }}>{leaveMessage}</Modal>
    </AdminLayout>
  );
}
