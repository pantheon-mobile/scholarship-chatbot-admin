...
const fallbackConfig: ChatUiConfig = {
  title: "東京理科大学奨学金問合せチャット", admin_title: "東京理科大学奨学金問合せチャット　管理サイト", header_icon_url: null,
  initial_message: "奨学金について知りたいことを入力してください。登録されている資料をもとに回答します。",
...
export default function ChatPage() {
  ...
  const [question, setQuestion] = useState(""); const [busy, setBusy] = useState(false); const [historyBusy, setHistoryBusy] = useState(false); const [openingHistory, setOpeningHistory] = useState(false); const [historyActionBusy, setHistoryActionBusy] = useState(false); const [historyActionError, setHistoryActionError] = useState(""); const [error, setError] = useState("");
  ...
  const nextSequence = useRef(1); const chatSessionId = useRef(crypto.randomUUID()); const chatStarted = useRef(false); const messagesRef = useRef<HTMLDivElement>(null); const composingQuestion = useRef(false);
  ...
  async function handleSubmit(event: FormEvent) {
    event.preventDefault(); const normalized = question.trim(); if (!normalized || busy || openingHistory || !identifier || config.maintenance_enabled) return;
    setQuestion(""); setError(""); setBusy(true); const submittedAt = now(); const interactionId = crypto.randomUUID(); const sequence = nextSequence.current++;
    ...
    try {
      ...
      const result = await sendChatMessage(normalized, chatSessionId.current, interactionId); const answeredAt = now();
  ...
  function openFeedback(messageId: string, interactionId: string, rating: "GOOD" | "BAD") { setFeedback({ messageId, interactionId, rating }); const saved = messages.find((item) => item.id === messageId && item.rating === rating); setFeedbackOption(saved?.feedbackReason ?? ""); setFeedbackComment(saved?.feedbackComment ?? ""); setFeedbackError(""); }
  async function saveFeedback() {
    if (!feedback) return; setFeedbackError(""); const comment = feedbackComment.trim();
    if ([feedbackOption, comment].filter(Boolean).join("：").length > 1000) { setFeedbackError("理由とコメントは合わせて1000文字以内で入力してください。"); return; }
    setFeedbackBusy(true);
    try { await submitFeedback(feedback.interactionId, feedback.rating, comment, feedbackOption); setMessages((current) => current.map((item) => item.id === feedback.messageId ? { ...item, rating: feedback.rating, feedbackReason: feedbackOption, feedbackComment: comment } : item)); setFeedback(undefined); }
    catch (reason) { setFeedbackError(reason instanceof Error ? reason.message : "評価を保存できませんでした。"); } finally { setFeedbackBusy(false); }
  }
  function newChat() { if (busy || openingHistory) return; chatSessionId.current = crypto.randomUUID(); chatStarted.current = false; nextSequence.current = 1; setMessages([]); setError(""); setMobileSidebarOpen(false); }
  function toggleSidebar() {
    if (window.matchMedia("(max-width: 760px)").matches) setMobileSidebarOpen((value) => !value);
    else setSidebarOpen((value) => !value);
  }
  async function openHistory(id: string) {
    if (busy || openingHistory) return;
    setOpeningHistory(true); setHistoryBusy(true); setError(""); try { const detail = await fetchChatHistoryDetail(id); chatSessionId.current = id; chatStarted.current = true; nextSequence.current = detail.next_sequence_number ?? (detail.messages.filter((item) => item.role === "user").length + 1); setMessages(detail.messages.map((item) => ({ id: item.id, role: item.role, content: item.content, sentAt: item.sent_at, citations: item.citations, interactionId: item.interaction_id || undefined, rating: item.rating || undefined, feedbackReason: item.feedback_reason ?? "", feedbackComment: item.feedback_comment ?? "", answerType: item.answer_type || undefined }))); setMobileSidebarOpen(false); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "履歴を取得できませんでした。"); } finally { setHistoryBusy(false); setOpeningHistory(false); }
  }
  ...
  return <main className={styles.main} style={{ "--chat-frame-color": config.frame_color } as CSSProperties}>
    <header className={styles.header}><button type="button" className={styles.mobileMenuTrigger} aria-label="モバイルメニューを開閉" aria-expanded={mobileSidebarOpen} onClick={toggleSidebar}><AdminIcon name="menu" size={26} /></button><div className={styles.brand}><span className={styles.schoolIcon}>{config.header_icon_url ? <Image className={styles.schoolIconImage} src={config.header_icon_url} alt="" width={32} height={32} unoptimized /> : <AdminIcon name="university" size={32} />}</span><h1>{config.title}</h1></div><button type="button" className={styles.adminLink} onClick={() => router.push("/")}>管理サイト</button><div className={styles.userWrap}><button type="button" className={styles.userButton} onClick={() => setUserMenuOpen((value) => !value)}>{user?.display_name || user?.subject} ▾</button>{userMenuOpen && <div className={styles.userMenu}><span>ID：{user?.subject}</span><button type="button" onClick={() => router.push("/")}>閉じる</button></div>}</div></header>
    {mobileSidebarOpen && <button className={styles.scrim} aria-label="メニューを閉じる" onClick={() => setMobileSidebarOpen(false)} />}
    <aside className={`${styles.sidebar} ${sidebarOpen ? "" : styles.sidebarCollapsed} ${mobileSidebarOpen ? styles.sidebarMobileOpen : ""}`}><button type="button" className={styles.sidebarMenuTrigger} aria-label={sidebarOpen ? "サイドメニューを閉じる" : "サイドメニューを開く"} aria-expanded={sidebarOpen} onClick={toggleSidebar}><AdminIcon name="menu" size={34} /></button><div className={styles.sidebarBody}><button type="button" className={styles.newChat} disabled={busy || openingHistory} onClick={newChat}><span className={styles.sidebarIcon}><AdminIcon name="plus" size={30} /></span><b>新しいチャット</b></button>{config.history_enabled && <><button type="button" className={`${styles.historyToggle} ${historyPanelOpen ? styles.historyToggleActive : ""}`} aria-expanded={historyPanelOpen} onClick={() => { setHistoryPanelOpen(true); setHistoryMenuId(undefined); setEditingHistory(undefined); }}><span className={styles.sidebarIcon}><AdminIcon name="chat" size={30} /></span><b>チャット履歴</b></button><div className={styles.historyPanel}>{historyPanelOpen && <label className={styles.historySearch}><button type="button" className={styles.historySearchBack} aria-label="検索を終了" onClick={() => { setHistoryPanelOpen(false); setHistorySearch(""); setHistoryMenuId(undefined); setEditingHistory(undefined); void reloadHistory(""); }}><AdminIcon name="back" size={20} /></button><span className={styles.visuallyHidden}>チャットを検索</span><input autoFocus value={historySearch} placeholder="チャットを検索" onChange={(event) => setHistorySearch(event.target.value)} /></label>}<div className={styles.recent}><h2>{historyPanelOpen ? (historySearch.trim() ? "検索結果" : "チャット履歴") : "最近の履歴"}</h2>{historyBusy && <p>読み込み中…</p>}{!historyBusy && histories.map((item) => <div className={styles.historyRow} key={item.id}>{editingHistory?.id === item.id ? <div className={styles.historyEdit}><input aria-label="チャット名" maxLength={100} value={editingHistory.title} disabled={historyActionBusy} onChange={(event) => setEditingHistory({ ...editingHistory, title: event.target.value })} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void saveHistoryTitle(); } if (event.key === "Escape") setEditingHistory(undefined); }} /><button type="button" onClick={() => setEditingHistory(undefined)}>キャンセル</button></div> : <><button type="button" className={styles.historyOpen} disabled={busy || openingHistory} title={item.title} onClick={() => void openHistory(item.id)}><span>{item.title}</span></button><button type="button" className={styles.historyMore} disabled={busy || openingHistory} aria-label={`${item.title}のメニュー`} aria-expanded={historyMenuId === item.id} onClick={() => setHistoryMenuId((value) => value === item.id ? undefined : item.id)}>⋮</button>{historyMenuId === item.id && <div className={styles.historyActions}><button type="button" onClick={() => { setEditingHistory({ id: item.id, title: item.title }); setHistoryMenuId(undefined); }}>編集</button><button type="button" className={styles.historyDelete} onClick={() => { setDeleteHistoryTarget(item); setHistoryMenuId(undefined); }}>削除</button></div>}</>}</div>)}{!historyBusy && histories.length === 0 && <p>{historyPanelOpen && historySearch.trim() ? "該当するチャットはありません" : "履歴はありません"}</p>}</div></div></>}</div></aside>
    <section className={`${styles.chat} ${sidebarOpen ? "" : styles.chatExpanded}`} aria-label="チャット"><div ref={messagesRef} className={styles.messages} aria-live="polite">{config.maintenance_enabled && <div className={styles.maintenance}>{config.maintenance_message}</div>}{messages.map((message) => <article key={message.id} className={`${styles.messageRow} ${message.role === "user" ? styles.userRow : styles.assistantRow}`}><div className={`${styles.message} ${message.role === "user" ? styles.userMessage : styles.assistantMessage}`}><div className={styles.visuallyHidden}>{message.role === "user" ? "あなた" : "チャットボット"}</div>{message.contextReference && <p className={styles.contextNotice}>過去のチャット「{message.contextReference}」を参照しています。</p>}{message.role === "assistant" && <span className={styles.botIcon}>{config.bot_icon_url ? <Image src={config.bot_icon_url} alt="" width={24} height={24} unoptimized /> : <AdminIcon name="bot" size={24} />}</span>}{message.role === "assistant" ? <MarkdownAnswer content={message.content} /> : <p>{message.content}</p>}{message.answerType !== "NO_ANSWER" && !!message.citations?.length && <div className={styles.citations}><b>【参照元】</b><ul>{message.citations.map((citation, index) => <li key={`${citation.title}-${index}`}>{safeCitationUrl(citation.uri) ? <a href={safeCitationUrl(citation.uri)!} target="_blank" rel="noopener noreferrer">{citation.title}</a> : citation.title}</li>)}</ul></div>}<time dateTime={message.sentAt}>{formatTimestamp(message.sentAt)}</time>{message.interactionId && <div className={styles.feedback}><span className={styles.feedbackLabel}>回答は役に立ちましたか？</span><div className={styles.feedbackButtons}><button aria-pressed={message.rating === "GOOD"} className={message.rating === "GOOD" ? styles.selected : ""} onClick={() => openFeedback(message.id, message.interactionId!, "GOOD")}><AdminIcon name="thumbUp" size={20} />Good</button><button aria-pressed={message.rating === "BAD"} className={message.rating === "BAD" ? styles.selected : ""} onClick={() => openFeedback(message.id, message.interactionId!, "BAD")}><AdminIcon name="thumbDown" size={20} />Bad</button><button className={styles.copy} onClick={() => void copyAnswer(message)} aria-label="回答をコピー"><AdminIcon name="copy" size={20} />{copiedId === message.id ? "コピー済み" : "コピー"}</button></div></div>}</div></article>)}{busy && <article className={`${styles.messageRow} ${styles.assistantRow}`}><div className={`${styles.message} ${styles.assistantMessage}`}><div className={styles.role}>チャットボット</div><p className={styles.thinking}>回答を作成しています<span>…</span></p></div></article>}</div>
      {error && <p className={styles.error} role="alert">{error}</p>}<form className={styles.composer} onSubmit={handleSubmit}><label htmlFor="chat-question" className={styles.visuallyHidden}>質問</label><textarea id="chat-question" value={question} maxLength={config.question_max_length} rows={1} placeholder={config.input_placeholder} disabled={busy || config.maintenance_enabled} onChange={(event) => setQuestion(event.target.value)} onCompositionStart={() => { composingQuestion.current = true; }} onCompositionEnd={() => { composingQuestion.current = false; }} onKeyDown={(event) => { const nativeEvent = event.nativeEvent as KeyboardEvent; if (event.key === "Enter" && !event.shiftKey && !composingQuestion.current && !nativeEvent.isComposing && nativeEvent.keyCode !== 229) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} /><button type="submit" disabled={busy || openingHistory || !question.trim() || config.maintenance_enabled}>送信</button></form><p className={styles.notice}>生成AIの回答には誤りが含まれる場合があります。重要な手続きは参照資料や担当窓口でもご確認ください。</p></section>
    <Modal open={Boolean(feedback)} title={feedback?.rating === "GOOD" ? "回答へのGood評価" : "回答へのBad評価"} confirmLabel={feedbackBusy ? "保存中…" : "送信する"} busy={feedbackBusy} error={feedbackError || undefined} onConfirm={() => void saveFeedback()} onClose={() => setFeedback(undefined)}><div className={styles.feedbackForm}><p>{feedback?.rating === "GOOD" ? config.good_message : config.bad_message}</p><label>理由（任意）<select value={feedbackOption} onChange={(event) => setFeedbackOption(event.target.value)}><option value="">選択してください</option>{feedbackOption && !options.includes(feedbackOption) && <option value={feedbackOption}>{feedbackOption}</option>}{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label><label>コメント（任意）<textarea maxLength={1000} rows={4} value={feedbackComment} onChange={(event) => setFeedbackComment(event.target.value)} /></label></div></Modal>
