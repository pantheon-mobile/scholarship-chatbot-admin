...
export async function fetchCategories(): Promise<CategoryListResponse> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/categories`, { cache: "no-store" });
...
export async function createCategory(values: CategoryWriteValues): Promise<Category> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/categories`, {
    method: "POST",
...
export async function updateCategory(id: number, values: CategoryWriteValues & { version: number }): Promise<Category> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/categories/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(values),
  });
...
export async function deleteCategory(id: number, version: number): Promise<void> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/categories/${id}?version=${version}`, { method: "DELETE" });
...
export async function bulkDeleteCategories(items: CategoryDeleteTarget[]): Promise<number> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/categories/bulk-delete`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function reorderCategories(parentId: number | null, items: CategoryDeleteTarget[]): Promise<Category[]> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/categories/order`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function exportCategories(): Promise<{ blob: Blob; fileName: string }> {
  const response = await authenticatedFetch(`${apiBase}/api/v1/categories/export`);
