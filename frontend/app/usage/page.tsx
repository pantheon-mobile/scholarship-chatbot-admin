...
import { downloadUsageFile } from "@/lib/reportingApi";
...
export default function UsagePage() {
  ...
  const download = async (kind: "users" | "access-logs" | "operation-logs") => {
    ...
    try { await downloadUsageFile(kind, from, to, filters); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "ファイルを取得できませんでした。"); }
    finally { setBusy(""); }
  };
  return <AdminLayout activeMenu="usage" contentWidth="default" contentAlign="start" onNavigate={(href) => router.push(href)}>
    ...
    <section className={styles.section}>
      <h2>【ユーザリスト】</h2>
      <div className={styles.userFilters}><SelectField label="権限：" value={userRole} onChange={(event) => setUserRole(event.target.value)}><option value="">（全て）</option><option value="staff">職員</option><option value="admin">システム管理者</option></SelectField></div>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.grid}>
        ...
        <SelectField label="権限：" value={accessRole} onChange={(event) => setAccessRole(event.target.value)}><option value="">（全て）</option><option value="staff">職員</option><option value="admin">システム管理者</option></SelectField>
        <FormField label="ログインID：" value={accessUserIds} placeholder="カンマ区切りで複数指定" onChange={(event) => setAccessUserIds(event.target.value)} />
      </div>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.grid}>
        ...
        <SelectField label="権限：" value={operationRole} onChange={(event) => setOperationRole(event.target.value)}><option value="">（全て）</option><option value="staff">職員</option><option value="admin">システム管理者</option></SelectField>
        <FormField label="ログインID：" value={operationUserIds} placeholder="カンマ区切りで複数指定" onChange={(event) => setOperationUserIds(event.target.value)} />
      </div>
